...
def generate_debauchery(character: FatalModel):
    ...
    if character.race in ["Black Dwarf", "Brown Dwarf", "White Dwarf", "Dark Elf", "Light Elf", "Ogre", "Cliff Ogre", "Gruagach Ogre", "Kinder-fresser Ogre"]:
        debauchery_roll = debauchery_roll - 10
    if character.race in ["Kobold", "Borbytingarna Troll", "Hill Troll", "Subterranean Troll"]:
        debauchery_roll = debauchery_roll + 10
...
def generate_disposition(character: FatalModel):
    ...
    if len(character.morality.split(' ')) == 1:
        ...
        moral_mods = [character.morality.split(' ')[0][0], character.morality.split(' ')[-1][0]]
    
    if len(character.ethicality.split(' ')) == 1:
        ethical_mods = [character.ethicality[0]]
    else:
        ethical_mods = [character.ethicality.split(' ')[0][0], character.ethicality.split(' ')[-1][0]]

    if len(moral_mods) == 1 and len(ethical_mods) == 1:
        character.disposition = f"{ethical_mods[0]}{moral_mods[0]}"
    elif len(moral_mods) == 1:
        character.disposition = f"{ethical_mods[0]}{moral_mods[0]} w/ {ethical_mods[1]}{moral_mods[0]}"
    elif len(ethical_mods):
        character.disposition = f"{ethical_mods[0]}{moral_mods[0]} w/ {ethical_mods[0]}{moral_mods[1]}"
    else:
        character.disposition = f"{ethical_mods[0]}{moral_mods[0]} w/ {ethical_mods[1]}{moral_mods[1]}"
...
def generate_temperment(character: FatalModel):
    ...
    temp_dict = {character.sanguine: "sanguine",
...
def generate_piety(character: FatalModel):
    ...
    if character.race not in ['Black Dwarf', 'Brown Dwarf', 'White Dwarf', 'Dark Elf', 'Light Elf']:
        age_mod_chance = d4()
        if age_mod_chance > 1:
            race = character.race
