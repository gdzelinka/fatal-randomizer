...
def calculate_sub_abilities(character: FatalModel):
    ...
    character.bodily_attractiveness = character.original_bodily_attractiveness = (
        ability_dice()
    )
    ...
    character.hand_eye_coordination = character.original_hand_eye_coordination = (
        ability_dice()
    )
...
def calculate_main_abilities(character: FatalModel):
    character.physique = floor(
        (
            character.physical_fitness
            + character.strength
    ...
    character.charisma = floor(
        (character.facial + character.vocal + character.kinetic + character.rhetorical)
        / 4
    )
    ...
    character.dexterity = floor(
        (
            character.hand_eye_coordination
            + character.agility
            + character.reaction_speed
            + character.ennunciation
        )
        / 4
    )
    ...
    character.intelligence = floor(
        (character.language + character.math + character.analytic + character.spatial)
        / 4
    )
...
def reroll_subability(character: FatalModel, ability_name):
    if character.num_skill_rerolls < 4:
        ...
        modifiers = getattr(character, ability_name) + getattr(
            character, f"original_{ability_name}"
        )
...
def apply_subability_modifiers(character: FatalModel):
    ...
    character.sprint = fitness_table[ceil((character.physical_fitness / 6))][
        height_marker
    ]
    ...
    character.bodily_attractiveness_modifier = skill_modifier[
        ceil(character.bodily_attractiveness / 6)
    ]
