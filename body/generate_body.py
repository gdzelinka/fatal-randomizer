...
def generate_age(character: FatalModel):
    ...
    if character.race in ["Dark Elf", "Light Elf"]:
        ...
        character.age = random.randint(
            floor(character.elf_lifespan * 0.26), floor(character.elf_lifespan * 0.4)
        )
...
def apply_age_modifiers(character: FatalModel):
    ...
    if character.age == 0:
        ...
        character.foot_size = (
            f"{floor(int(feet[0])* 0.2)}-{floor(int(feet[1])*0.2)} inches"
        )
    ...
    if character.stage_of_life == "Infant":
        ...
        feet = character.foot_size.split(" ")[0].split("-")
        character.foot_size = (
            f"{floor(int(feet[0])* 0.4)}-{floor(int(feet[1])*0.4)} inches"
        )
        ...
        feet = character.foot_size.split(" ")[0].split("-")
        character.foot_size = (
            f"{floor(int(feet[0])* 0.8)}-{floor(int(feet[1])*0.8)} inches"
        )
...
def generate_best_worst_features(character: FatalModel):
    ...
    setattr(
        ...
        (getattr(character, feature_table[most_attractive]) + d10()),
    )
    if character.gender == "male" and most_attractive == "Crotch":
        character.manhood_length = floor(
            character.manhood_length * float(f"1.{(d10()):02d}")
        )
    ...
    setattr(
        ...
        (getattr(character, feature_table[least_attractive]) - d10()),
    )
    if character.gender == "male" and least_attractive == "Crotch":
        character.manhood_length = floor(
            character.manhood_length * float(f"0.{(100 - d10()):02d}")
        )
...
def generate_eye_color(character: FatalModel):
    character.eyes = random.choices(
        population=eye_color[character.race], weights=[5, 65, 10, 19, 1], k=1
    )[0]
    return character
...
def generate_vision(character: FatalModel):
    ...
    if character.vision == "Near-sightedness":
        character.aim.skill_modifier = (
            character.aim.skill_modifier + vision_imparement[vision_roll]
        )
        character.hurl.skill_modifier = (
            character.hurl.skill_modifier + vision_imparement[vision_roll]
        )
        character.mounted_archery.skill_modifier = (
            character.mounted_archery.skill_modifier + vision_imparement[vision_roll]
        )
        character.search.skill_modifier = (
            character.search.skill_modifier + vision_imparement[vision_roll]
        )
        character.sight.skill_modifier = (
            character.sight.skill_modifier + vision_imparement[vision_roll]
        )
...
def generate_sexual_features(character: FatalModel, size_mod: int = None):
    ...
    character.areola_diameter = areola_diameter[character.race][
        ceil(areola_diameter_roll / 5)
    ]

    character.areola_hue = random.choices(
        population=list(areola_hue.keys()), weights=list(areola_hue.values()), k=1
    )[0]
    ...
    if anal_circ_roll > 98:
        ...
        character.anal_circumference_potential = circumference[character.race][
            ceil(anal_circ_roll / 10)
        ]

    depth_percent = d20() + d20() + d20()
    character.anal_depth_potential = (character.height / 12) * float(
        f"0.{depth_percent:02d}"
    )
    ...
    if character.gender == "female":
        ...
        character.bodily_attractiveness = (
            character.bodily_attractiveness + cup_size[cup_size_roll][1]
        )
        ...
        if vag_circ_roll > 98:
            character.vaginal_circumference_potential = circumference[character.race][
                11
            ]
        else:
            character.vaginal_circumference_potential = circumference[character.race][
        ...
        character.vaginal_depth_potential = (character.height / 12) * float(
            f"0.{depth_percent:02d}"
        )
...
def generate_manhood(character: FatalModel):
    if character.gender == "male":
        ...
        character.manhood_circumference = (
            character.manhood_length * 0.85 * dick_size[circ_roll]
        )
...
def generate_rare_features(character: FatalModel):
    ...
    if foot_roll > 98:
        ...
        character.foot_size = foot_length[character.race][character.gender][
            ceil(foot_roll / 10)
        ]
...
def generate_bpp(character: FatalModel):
    ...
    character.head.bpp = (
        floor(character.life_points * proportion_table["Head"])
        if floor(character.life_points * proportion_table["Head"]) >= 1
        else 1
    )
    character.face.bpp = (
        floor(character.life_points * proportion_table["Face"])
        if floor(character.life_points * proportion_table["Face"]) >= 1
        else 1
    )
    character.upper_torso.bpp = (
        floor(character.life_points * proportion_table["Upper Torso"])
        if floor(character.life_points * proportion_table["Upper Torso"]) >= 1
        else 1
    )
    character.lower_torso.bpp = (
        floor(character.life_points * proportion_table["Lower Torso"])
        if floor(character.life_points * proportion_table["Lower Torso"]) >= 1
        else 1
    )
    character.groin.bpp = (
        floor(character.life_points * proportion_table["Groin"])
        if floor(character.life_points * proportion_table["Groin"]) >= 1
        else 1
    )
    character.upper_right_arm.bpp = (
        floor(character.life_points * proportion_table["Upper Right Arm"])
        if floor(character.life_points * proportion_table["Upper Right Arm"]) >= 1
        else 1
    )
    character.lower_right_arm.bpp = (
        floor(character.life_points * proportion_table["Lower Right Arm"])
        if floor(character.life_points * proportion_table["Lower Right Arm"]) >= 1
        else 1
    )
    character.right_hand.bpp = (
        floor(character.life_points * proportion_table["Right Hand"])
        if floor(character.life_points * proportion_table["Right Hand"]) >= 1
        else 1
    )
    character.upper_left_arm.bpp = (
        floor(character.life_points * proportion_table["Upper Left Arm"])
        if floor(character.life_points * proportion_table["Upper Left Arm"]) >= 1
        else 1
    )
    character.lower_left_arm.bpp = (
        floor(character.life_points * proportion_table["Lower Left Arm"])
        if floor(character.life_points * proportion_table["Lower Left Arm"]) >= 1
        else 1
    )
    character.left_hand.bpp = (
        floor(character.life_points * proportion_table["Left Hand"])
        if floor(character.life_points * proportion_table["Left Hand"]) >= 1
        else 1
    )
    character.upper_right_leg.bpp = (
        floor(character.life_points * proportion_table["Upper Right Leg"])
        if floor(character.life_points * proportion_table["Upper Right Leg"]) >= 1
        else 1
    )
    character.lower_right_leg.bpp = (
        floor(character.life_points * proportion_table["Lower Right Leg"])
        if floor(character.life_points * proportion_table["Lower Right Leg"]) >= 1
        else 1
    )
    character.right_foot.bpp = (
        floor(character.life_points * proportion_table["Right Foot"])
        if floor(character.life_points * proportion_table["Right Foot"]) >= 1
        else 1
    )
    character.upper_left_leg.bpp = (
        floor(character.life_points * proportion_table["Upper Left Leg"])
        if floor(character.life_points * proportion_table["Upper Left Leg"]) >= 1
        else 1
    )
    character.lower_left_leg.bpp = (
        floor(character.life_points * proportion_table["Lower Left Leg"])
        if floor(character.life_points * proportion_table["Lower Left Leg"]) >= 1
        else 1
    )
    character.left_foot.bpp = (
        floor(character.life_points * proportion_table["Left Foot"])
        if floor(character.life_points * proportion_table["Left Foot"]) >= 1
        else 1
    )
